...
@pytest.fixture
def user_two():
    """Create a second user to use in tests."""
...
def api_client(user, request, authenticated):
    ...
    # Check if the test requires an authenticated client
    if authenticated:
...
@pytest.fixture
def api_client_user_two(user_two, request, authenticated):
    client = APIClient()
    if authenticated:
...
def test_event_list_view(api_client, user, event, authenticated):
...
def test_event_list_view_forbidden(api_client, authenticated):
...
@pytest.mark.django_db
@pytest.mark.parametrize('authenticated', [True])  # This will create an authenticated client
def test_create_event(api_client, user, authenticated):
    """Test that a user can create a new event.

    Expected behavior: The API should return a 201 Created status code and the response should contain the created event's details.
    """
    data = {
        'title': 'Post Event',
...
@pytest.mark.django_db
@pytest.mark.parametrize('authenticated', [False])  # This will create an unauthenticated client
def test_unauthenticated_user_cannot_create_event(api_client, authenticated):
...
@pytest.mark.django_db
@pytest.mark.parametrize('authenticated', [True])  # This will create an authenticated client
def test_invalid_date_format(user, api_client, authenticated):
...
@pytest.mark.django_db
@pytest.mark.parametrize('authenticated', [True])  # This will create an authenticated client
def test_user_cannot_create_empty_event(api_client, user, authenticated):
...
@pytest.mark.django_db
@pytest.mark.parametrize('authenticated', [True])  # This will create an authenticated client
def test_update_event(api_client, user, event, authenticated):
...
@pytest.mark.django_db
@pytest.mark.parametrize('authenticated', [True])  # This will create an authenticated client
def test_update_event_wrong_user(api_client_user_two, user_two, event, authenticated):
    ...
    url = reverse('event_update', args=[event.id])
    response = api_client_user_two.put(url, updated_data, format='json')
...
def test_update_non_existent_event(api_client, user, event, authenticated):
    event_id = 9999
...
@pytest.mark.django_db
@pytest.mark.parametrize('authenticated', [True])  # This will create an authenticated client
def test_delete_event(api_client, user, event, authenticated):
